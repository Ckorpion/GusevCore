   ...
   class _admin_scripts {
      public function init($page, $params) {
         ...
         while ($script = $result -> fetch_assoc()) {
            ...
            $script['isButtonStart'] = 'none';
            $script['isButtonStop'] = 'none';
            $script['isButtonReload'] = 'none';
            if ($script['status'] <= -2) {
               $script['isButtonStart'] = 'inline-block';
            }
            if ($script['status'] == -1) {
               $script['isButtonStop'] = 'inline-block';
            }
            if ($script['status'] == 0) {
               $script['isButtonStop'] = 'inline-block';
               $script['isButtonReload'] = 'inline-block';
            }

            $scripts .= $GC -> getPage('_admin/scripts?script', $script)['HTML'];
         ...
         $page['HTML'] = $GC -> getPage('_admin/scripts?scripts', array(
            'scripts' => $scripts,
            'countAll' => $countAll,
      ...
      public function getNewForm() {
         ...
         return $GC -> getPage('_admin/scripts?new', array(
            'caption' => 'Add script',
            'terminate' => 120,
      ...
      public function edit($data) {
         ...
         if (is_numeric($data['id'])) {
            ...
            if (isset($script['id'])) {
               ...
               return $GC -> getPage('_admin/scripts?new', $info)['HTML'];
            }
         }
      }
